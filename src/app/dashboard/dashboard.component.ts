...
import { LifeCycleUtil, MappingStates, ThreatMapping, ThreatSeverities, ThreatSeverityUtil, ThreatStates, ThreatStateUtil } from '../model/threat-model';
...
export class DashboardComponent implements AfterViewInit {
  ...
  constructor(public theme: ThemeService, public dataService: DataService, private translate: TranslateService,
    private locStorage: LocalStorageService, public dialog: DialogService, private router: Router) { 
  ...
  public UpdateDiagrams() {
    ...
    let dia1 = this.createThreatSummaryDiagram(wid, hei);
    let dia2 = this.createMitigationSummaryDiagram(wid, hei);
    let dia3 = this.createThreatPerLifecycleDiagram(wid, hei);
    let dia4 = this.createThreatPerTypeDiagram(wid, hei);
  ...
  private createThreatSummaryDiagram(wid, hei): IDiagramData {
    ...
    let getSeries = (mappings: ThreatMapping[]): ISerie[] => {
      ...
      res.push({ name: this.translate.instant('properties.threatstate.NotSet'), value: mappings.filter(x => !ThreatSeverityUtil.GetTypes().includes(x.Severity)).length });
      ThreatSeverityUtil.GetTypes().forEach(sev => {
        res.push({ name: this.translate.instant(ThreatSeverityUtil.ToString(sev)), value: mappings.filter(x => x.Severity == sev).length });
      });
      return res;
    };
    ...
    if (allThreatMappings.length > 0) {
      let data: IStackedSeries = {
        name: this.translate.instant('general.Other'),
    ...
    let dia: IDiagramData = {
      ...
      view: [wid, hei],
      scheme: this.isBlueColorScheme ? 'air' : { domain: [this.getNeutral(), DiaColors.Green, DiaColors.Yellow, DiaColors.Red, DiaColors.DarkRed] },
      xAxisLabel: this.translate.instant('pages.dashboard.ThreatSummary'),
      yAxisLabel: this.translate.instant('pages.dashboard.NumberOfThreats')
    };
    return dia;
  }

  private createMitigationSummaryDiagram(wid, hei): IDiagramData {
    let pf = this.dataService.Project;
    let diagramValues: IStackedSeries[] = [];
    ...
    let getSeries = (mappings: MitigationMapping[]): ISerie[] => {
      ...
      MitigationStateUtil.GetMitigationStates().filter(x => x != MitigationStates.NotApplicable).forEach(state => {
        res.push({ name: this.translate.instant(MitigationStateUtil.ToString(state)), value: mappings.filter(x => x.MitigationState == state).length });
      });
      return res;
    };
    ...
    let dia: IDiagramData = {
      results: diagramValues,
      view: [wid, hei],
      scheme: this.isBlueColorScheme ? 'air' : { domain: [DiaColors.DarkRed, this.getNeutral(), DiaColors.Red, DiaColors.Yellow, DiaColors.Green] },
      xAxisLabel: this.translate.instant('pages.dashboard.MitigationSummary'),
      yAxisLabel: this.translate.instant('pages.dashboard.NumberOfMitigations')
    };
    return dia;
  }

  private createThreatPerLifecycleDiagram(wid, hei): IDiagramData {
    let pf = this.dataService.Project;
    let diagramValues: IStackedSeries[] = [];
    let allThreatMappings = pf.GetThreatMappings();
    let getSeries = (mappings: ThreatMapping[]): ISerie[] => {
    ...
    LifeCycleUtil.GetKeys().forEach(lc => {
      ...
      allThreatMappings = allThreatMappings.filter(x => !mappings.includes(x));
      let data: IStackedSeries = {
        name: this.translate.instant(LifeCycleUtil.ToString(lc)),
    ...
    let dia: IDiagramData = {
      ...
      view: [wid, hei],
      scheme: this.isBlueColorScheme ? 'air' : { domain: [this.getNeutral(), DiaColors.Green, DiaColors.Yellow, DiaColors.Red, DiaColors.DarkRed] },
      xAxisLabel: this.translate.instant('pages.dashboard.ThreatPerLifecycle'),
      yAxisLabel: this.translate.instant('pages.dashboard.NumberOfThreats')
    };
    return dia;
  }

  private createThreatPerTypeDiagram(wid, hei): IDiagramData {
    let pf = this.dataService.Project;
    let diagramValues: IStackedSeries[] = [];
    let allThreatMappings = pf.GetThreatMappings();
    let getSeries = (mappings: ThreatMapping[]): ISerie[] => {
    ...
    let data: IStackedSeries = {
      name: this.translate.instant('general.Hardware'),
    ...
    data = {
      name: this.translate.instant('general.Software'),
    ...
    data = {
      name: this.translate.instant('general.Process'),
    ...
    data = {
      name: this.translate.instant('general.DataFlow'),
    ...
    let dia: IDiagramData = {
      ...
      view: [wid, hei],
      scheme: this.isBlueColorScheme ? 'air' : { domain: [this.getNeutral(), DiaColors.Green, DiaColors.Yellow, DiaColors.Red, DiaColors.DarkRed] },
      xAxisLabel: this.translate.instant('pages.dashboard.ThreatPerType'),
      yAxisLabel: this.translate.instant('pages.dashboard.NumberOfThreats')
    };
    return dia;
  }
  ...
  private getNeutral() {
    if (this.theme.IsDarkMode) return DiaColors.White;
